...
function createRegistry({ pool, getConfig, secret = process.env.JWT_SECRET, validate = validateConnection }) {
  ...
  async function assertUnique(candidate, exceptId) {
    ...
    if (all.some(server => server.id !== exceptId && server.upstreamId === info.id)) {
      throw new RegistryError('This Silo server is already configured under another address.', 409);
    }
    ...
    if (first) {
      ...
      if (info.id === primaryInfo.id) throw new RegistryError('The primary Silo server is already configured.', 409);
    }
    return info;
  }

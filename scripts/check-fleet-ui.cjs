...
async function main() {
  ...
  try {
    ...
    const real = await context.request.get('http://127.0.0.1:3000/fleet', { headers: { Authorization: `Bearer ${token}` } });
    ...
    const session = (name, paused = false) => ({ Id: 'same-id', UserId: 'same-user', UserName: name,
      ...
      NowPlayingItem: { Id: 'same-item', Name: 'Fixture movie', Type: 'Movie', RunTimeTicks: 36000000000 },
    ...
    const snapshot = () => {
      const servers = [main, ...extras].map(server => {
        ...
        return { id: server.id, name: server.name, enabled: server.enabled, isPrimary: server.isPrimary,
          ...
          sessions: server.enabled ? [{ ...session(server.isPrimary ? 'Local viewer' : 'Remote viewer', !server.isPrimary), stale: bad }] : [] };
      });
    ...
    await page.goto('http://127.0.0.1:3000/settings/servers');

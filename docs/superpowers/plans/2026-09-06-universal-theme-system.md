...
- `borderCanvas`, `borderRaised`, `borderInset`, `borderStrong`, `borderNav`, and `borderOverlay` each begin at `mix(associatedSurface, associated normal text token, 0.42)` and are adjusted toward warm light or near-black until reaching 3:1 against canvas, raised, inset, interactive, nav, and overlay respectively. `borderStrong` retains its legacy name as the interactive-surface meaningful boundary; `borderSubtle = mix(surface, textRaised, 0.18)` remains decorative only.
...
- Fixed status candidates are live `#ff6f63`, success `#70b981`, warning `#ffa64f`, danger `#e45f55`, unavailable `#a0977f`; each is adjusted only when needed to reach 3:1 against `surfaceRaised`. `dangerInteractive` independently adjusts the danger candidate to 3:1 against `surfaceInteractive` for destructive controls with neutral interactive fills.
